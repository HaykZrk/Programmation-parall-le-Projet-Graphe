...
run-par-3000: graph_par $(DATADIR)/$(DATASET_3000) 
	@mkdir -p $(RESDIR_PAR)
	@echo "===== $@ OMP=$(OMP)======"
	for np in 4; do \
		echo ">>> -n $$np"; \
		mpirun -n $$np -hostfile hosts.txt ./graph_par -i $(DATADIR)/$(DATASET_3000) -t adj > $(RESDIR_PAR)/$(DATASET_3000)-$(MPI_EXT)$$np-$(TCLOS_EXT); \
		echo ; \
	done
...
run-omp-1000: graph_omp $(DATADIR)/$(DATASET_1000) 
	@mkdir -p $(RESDIR_OMP)
	@echo "===== $@ ======"
	./graph_omp -i $(DATADIR)/$(DATASET_1000) -t adj > $(RESDIR_OMP)/$(DATASET_1000)-$(TCLOS_EXT)

run-omp-3000: graph_omp $(DATADIR)/$(DATASET_3000)
	@mkdir -p $(RESDIR_OMP)
	@echo "===== $@ ======"
	./graph_omp -i $(DATADIR)/$(DATASET_3000) -t adj > $(RESDIR_OMP)/$(DATASET_3000)-$(TCLOS_EXT)

run-omp-10000: graph_omp $(DATADIR)/$(DATASET_10000) 
	@mkdir -p $(RESDIR_OMP)
	@echo "===== $@ ======"
	./graph_omp -i $(DATADIR)/$(DATASET_10000) -t adj > $(RESDIR_OMP)/$(DATASET_10000)-$(TCLOS_EXT)
...
test-par-3000: run-par-3000 run-seq-3000
...
test-par-10000: run-par-10000 run-seq-10000
...
test-omp-1000: run-omp-1000
	@echo "$(shell cmp $(RESDIR_OMP)/$(DATASET_1000)-$(MPI_EXT)*-$(TCLOS_EXT) $(RESDIR_SEQ)/$(DATASET_1000)-$(TCLOS_EXT))"

test-omp-3000: run-omp-3000 run-seq-3000
	@echo "$(shell cmp $(RESDIR_OMP)/$(DATASET_3000)-$(MPI_EXT)*-$(TCLOS_EXT) $(RESDIR_SEQ)/$(DATASET_3000)-$(TCLOS_EXT))"
	@echo "MD5"
	@echo "$(shell $(MD5) $(RESDIR_OMP)/$(DATASET_3000)-$(TCLOS_EXT) )"
	
test-omp-10000: run-omp-10000
	@echo "$(shell cmp $(RESDIR_OMP)/$(DATASET_10000)-$(MPI_EXT)*-$(TCLOS_EXT) $(RESDIR_SEQ)/$(DATASET_10000)-$(TCLOS_EXT))"

# ----------------- clean ------------------------------
clean:
	rm -rf *.o $(BINARIES) *.dSYM *~ results-*
